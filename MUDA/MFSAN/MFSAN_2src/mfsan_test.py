...
def train(model):
    ...
    for i in range(1, 2):
        ...
        #  TODO：except Exception as err ??? 是数据不够的时候再来一轮？
        try:
        ...
        # if i % log_interval == 0:
        # # 轮次、轮次进度、总损失、分类损失、mmd损失、disc loss。
...
# output.data.max(2, keepdim=True)[1]中的2表示，找第3维的最大值；[1]表示，output.data.max(2, keepdim=True)会返回一个数组，第一个是output数组中第3维度的最大值是多少，第二个是最大值的位置在哪里。
# view_as 根据输入参数张量的形状，改变调用张量的形状
